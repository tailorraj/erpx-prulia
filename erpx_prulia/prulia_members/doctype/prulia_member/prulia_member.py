...
class PRULIAMember(Document):
    ...
    def validate(self):
        ...
        if self.register_acknowledgement != 1:
            throw(_(
                "You did not click the 'Acknowledgement' checkbox. If you do agree to the terms and conditions, please ensure you check the box before continuing. "))
        ...
        #     labels.nric_number = 'IC Number',
        #     labels.cell_number = 'Mobile number',
        #     labels.email = 'Email'
        #     for attr in ['full_name', 'nric_number', 'cell_number', 'email']:
        #         if getattr(existing_member, attr) != getattr(new_member, attr):
        #             changed.append(
        #                 '<p>{0}: {1} -> {2}</p>'.format(getattr(labels, attr), getattr(existing_member, attr),
        #                                                 getattr(new_member, attr)))
        #     if len(changed) > 0:
        #         emails = frappe.get_all("PRULIA New Member Notification", fields=['email'])
        #         frappe.sendmail(
        #             recipients=','.join([str(i.email) for i in emails]),
        #             sender='pruliatech@prulia.org.my',
        #             subject='PRULIA Member Profile Change Notification',
        #             message='<p>PRULIA ID: {0}</p>'.format(self.prudential_id)
        #                     + '<p>Email: {0}<p>'.format(self.email)
        #                     + '<p>What\'s changed:</p>'
        #                     + ''.join(changed)
        #         )
        # else:
        ...
        #         recipients=','.join([str(i.email) for i in emails]),
        #         sender='pruliatech@prulia.org.my',
        #         subject='PRULIA New Member Creation Notification',
        #         message='<p>PRULIA ID: {0}</p>'.format(self.prudential_id)
        #                 + '<p>Email: {0}<p>'.format(self.email)
        #     )

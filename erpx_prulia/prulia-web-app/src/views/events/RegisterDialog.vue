      ...
      <v-card>
        ...
        <v-card-text class="px-2 pb-2 pt-0">
          <v-list dense class="pt-0">
            <v-list-item>
              <v-select
                v-model="eventData.meal_option"
                label="Meal Option"
                :items="MEAL_OPTIONS"
                :rules="[
                  () => !!eventData.meal_option || 'Meal options is required'
                ]"
              >
              </v-select>
            </v-list-item>
            <v-list-item v-if="currentEvent.display_shirt_option">
              <v-select
                v-model="eventData.shirt_size"
                label="Shirt Size"
                :items="SHIRT_SIZE"
                :rules="[
                  () => !!eventData.shirt_size || 'Shirt options is required'
                ]"
              >
              </v-select>
            </v-list-item>
            <v-list-item v-if="currentEvent.display_accomodation_option">
              <v-select
                v-model="eventData.accomodation"
                label="Accomodation"
                :items="ACCOMODATION"
                :rules="[
                  () =>
                    !!eventData.accomodation ||
            ...
            <v-list-item class="pt-2">
              <span class="caption" style="text-align: justify;">
                ...
                hereafter be altered. I hereby authorize Prudential Assurance
                Malaysia Berhad to debit by commission account for the
    ...
    onSubmit() {
      ...
      this.$store
        .dispatch('events/register', {
          ...
          meal: this.eventData.meal_option || null,
          shirt: this.eventData.shirt_size || null,
          accomodation: this.eventData.accomodation || null,
          pref_lang: this.eventData.pref_lang || null
        })
        .then(() => {
          this.loading = false
          this.model = false
